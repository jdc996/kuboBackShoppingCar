...
describe('getProducts', () => {
    ...
    beforeAll(async () => {
        ...
        testProd2 = await ProductModel.create(
            {
                image: "test 1.jpg",
                name: "test product",
    ...
    test('Return  products from the database', async () => {
        ...
        test('Return the products with category "test" from the database', async () => {
            const res = await requestTest(app)
                .get("/products?category=test&&category=test2")
            expect(res.status).toBe(200);
    ...
    test('return Categories of the product',async()=>{
        const res = await requestTest(app)
        .get("/products/categories/all/");
        expect(res.status).toBe(200);
        expect(res.body).not.toBeNull()
        expect(res.body).toEqual(["test","test2"])
    })
    afterAll(async () => {
        //await ProductModel.deleteMany({})
    })
